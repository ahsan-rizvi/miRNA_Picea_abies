#!/sw/apps/conda/latest/rackham_stage/bin/python3
#- Dr Ahsan Z Rizvi
#-------------------------
import numpy
import sys
import os
import numpy as np
#import matplotlib.pyplot as plt
from scipy import stats
from scipy.stats import mstats
import pandas as pd
#-------------------------


#MiRNA VST file read
#miRNA_name=list()
miRNA_exp=list()
with open(sys.argv[2]) as file:
    for line in file:
        #print(line)
        miRNA_exp.append(line)
        #temp1=line.split('\t')
        #miRNA_name.append(temp1)

#mRNA file read
#mRNA_name=list()
mRNA_exp=list()
with open(sys.argv[3]) as file:
    for line in file:
        #print(line)
        mRNA_exp.append(line)
        #temp1=line.split('\t')
        #mRNA_name.append(temp1)





#Select miRNA and mRNA significant pairs(E>4)
miRNA_pair=list()
mRNA_pair=list()
E_value=list()
i=0
total=0
total_le4=0
break1=0

with open(sys.argv[1]) as file:
    for line in file:
        temp1=(line.split('\t'))
        if len(temp1)>1 and i>1:
           total=total+1
           if float(temp1[2]) > 4:
              total_le4=total_le4+1   
              miRNA_pair.append(temp1[0])
              mRNA_pair.append(temp1[1])
              E_value.append(temp1[2])
        i=i+1
print('Total pairs present:',total)
print('Total pairs Evalue> 4:',total_le4)


temp23=sys.argv[1]+'.miRNA.txt'
print(temp23)
f  = open(temp23, "w")
miRNA_withTarget=np.unique(miRNA_pair)
print('Total miRNAs with targets: ',len(miRNA_withTarget))
for i in range(0,len(miRNA_withTarget)):
    #print(miRNA_withTarget[i])
    f.writelines(miRNA_withTarget[i]+'\n')
f.close()

#sys.exit()
############################################
#correlation check
temp22=sys.argv[4]
f  = open(temp22, "w")
print('list of miRNA-mRNA pair passed the CC test: ',temp22)
#temp33=sys.argv[4]+'.log'
#f2  = open(temp33, "w")
Count_CC2=0
Count_CC=0
CC_Pvalue_miRNA=list()
CC_Pvalue_mRNA=list()
for i in range(0,len(miRNA_pair)):
    break1=break1+1
    ###############
    #if break1>50:
     #  break
    ###############
    temp1=miRNA_pair[i]
    temp2=mRNA_pair[i]
    #print(temp1,temp2)
    miRNAid_in_Target=''
    mRNAid_in_Target=''
    for j in range(0,len(miRNA_exp)):    
        temp3=miRNA_exp[j].split('\t')
        temp4=temp3[0].split(',')
        miRNAid_in_Target=temp4[0]
        if miRNAid_in_Target==temp1:
           miRNAid_in_Target_matched=miRNA_exp[j]
           #print(miRNAid_in_Target_matched)
    for k in range(0,len(mRNA_exp)):
        temp5=mRNA_exp[k].split('\t')
        mRNAid_in_Target=temp5[0]
        if temp2==mRNAid_in_Target: 
           mRNAid_in_Target_matched=mRNA_exp[k]
           #print(mRNA_exp[k])
    #print('---------------------------------------------------------------------')
    miRNA_temp=miRNAid_in_Target_matched.split('\n')
    miRNA_temp2=miRNA_temp[0].split('\t')

    miRNA_VST=[float(miRNA_temp2[1]),float(miRNA_temp2[2]),float(miRNA_temp2[3]),float(miRNA_temp2[4]),float(miRNA_temp2[5]),float(miRNA_temp2[6]), float(miRNA_temp2[7]),float(miRNA_temp2[8]),float(miRNA_temp2[9]),float(miRNA_temp2[10]),float(miRNA_temp2[11]),float(miRNA_temp2[12]),  float(miRNA_temp2[13]),float(miRNA_temp2[14]),float(miRNA_temp2[15]),float(miRNA_temp2[16]),float(miRNA_temp2[17]),float(miRNA_temp2[18]), float(miRNA_temp2[19]),float(miRNA_temp2[20]),float(miRNA_temp2[21]),float(miRNA_temp2[22]),float(miRNA_temp2[23])]

    #print('##########################')
    mRNA_temp=mRNAid_in_Target_matched.split('\n')     
    mRNA_temp2=mRNA_temp[0].split('\t')

    mRNA_VST=[float(mRNA_temp2[1]),float(mRNA_temp2[2]),float(mRNA_temp2[3]),float(mRNA_temp2[4]),float(mRNA_temp2[5]),float(mRNA_temp2[6]), float(mRNA_temp2[7]),float(mRNA_temp2[8]),float(mRNA_temp2[9]),float(mRNA_temp2[10]),float(mRNA_temp2[11]),float(mRNA_temp2[12]),  float(mRNA_temp2[13]),float(mRNA_temp2[14]),float(mRNA_temp2[15]),float(mRNA_temp2[16]),float(mRNA_temp2[17]),float(mRNA_temp2[18]), float(mRNA_temp2[19]),float(mRNA_temp2[20]),float(mRNA_temp2[21]),float(mRNA_temp2[22]),float(mRNA_temp2[23])]

    #-------------------------------------------------
    [Corr,P_value]=mstats.pearsonr(mRNA_VST,miRNA_VST)
    P_value_lim=0.05
    if Corr<0:
       Count_CC2=Count_CC2+1
    if Corr<0 and P_value < P_value_lim:
       #print(miRNA_temp2[0],mRNA_temp2[0],E_value[i],Corr,P_value)
       #f.writelines(miRNA_temp2[0]+'\t'+mRNA_temp2[0]+'\n')
       f.writelines(miRNA_temp2[0]+'\t'+mRNA_temp2[0]+'\t'+E_value[i]+'\t'+str(Corr)+'\t'+str(P_value)+'\n')
       #f2.writelines(miRNA_temp2[0]+'\t'+mRNA_temp2[0]+'\t'+E_value[i]+'\t'+str(Corr)+'\t'+str(P_value)+'\n')
       CC_Pvalue_miRNA.append(miRNA_temp2[0])
       CC_Pvalue_mRNA.append(mRNA_temp2[0])
       Count_CC=Count_CC+1
    #-------------------------------------------------
       
f.close()
########################################
print('Mirna-mRNA pairs in negative CC values: ',Count_CC2) 
print('P value cut off: ', P_value_lim)        
print('Mirna-mRNA pairs in negative CC values and passed P value test: ',Count_CC)   
#print('Logfile is written at: ',temp33)     





#Check Frequency
#miRNA_uniq,miRNA_counts = numpy.unique(miRNA_pair,return_counts=True)
#print(miRNA_uniq)
#print(miRNA_counts)








