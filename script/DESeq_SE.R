...
d          <- DESeq(d)
fileP=paste(args[3], "PCA_SE.png", sep="")
png(file = fileP,width = 500,height = 500)
...
file3=paste(args[3], "SE_Log2FC_miRNA.csv", sep="")
...
##############################################
Uniq       = unique(temp)
file2=paste(args[3], "DGE_SE.txt", sep="")
write.table(Uniq, file=file2, quote=F,sep="\t", col.names = F, row.names = F)
colnames(data1)=a7[,2]
file3=paste(args[3], "DGE_SE_VST.tsv", sep="")
